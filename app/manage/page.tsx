...
function UploadModal({ name, onClose }: { name:string; onClose:()=>void }) {
  return (
    <div onClick={onClose} style={{ position:"fixed", inset:0, background:"rgba(0,0,0,0.45)",
      ...
      <div onClick={e=>e.stopPropagation()} style={{ background:"#fff", borderRadius:24,
        padding:"28px 24px", maxWidth:420, width:"100%", boxShadow:"0 20px 60px rgba(0,0,0,0.2)" }}>
        <div style={{ fontSize:36, textAlign:"center", marginBottom:8 }}>📂</div>
        <h2 style={{ margin:"0 0 6px", fontSize:18, fontWeight:900, color:"#1E3A8A", textAlign:"center" }}>
          Upload New Plan
        </h2>
        <p style={{ fontSize:13, color:"#64748B", textAlign:"center", marginBottom:20, lineHeight:1.6 }}>
          Uploading a new plan for <strong style={{ color:"#1E3A8A" }}>{name}</strong>.
        </p>
        <div style={{ border:`2px dashed ${BL}`, borderRadius:16, padding:"24px 16px",
          textAlign:"center", background:BL_L, marginBottom:16 }}>
          <div style={{ fontSize:28, marginBottom:6 }}>☁️</div>
          <div style={{ fontSize:14, color:BL, fontWeight:700 }}>Click to choose a file</div>
          <div style={{ fontSize:12, color:"#94A3B8", marginTop:4 }}>JSON format · .json only</div>
        </div>
        <div style={{ background:"#FFFBEB", border:"1px solid #FDE68A", borderRadius:12,
          padding:"10px 14px", fontSize:13, color:"#92400E", marginBottom:20, lineHeight:1.6 }}>
          💡 <strong>Coming soon.</strong> Send the file to your tech team to replace it on the server.
        </div>
        <button onClick={onClose} style={{ width:"100%", background:BL, color:"#fff", border:"none",
...
function ReadingSection({ src }: { src: DataSource }) {
  const [showUpload, setShowUpload] = useState(false);
  const rm = refillMonth(src.endDate);

  return (
    <>
      {showUpload && <UploadModal name="Bible Reading Plan" onClose={()=>setShowUpload(false)} />}
      <div>
...
function ScheduleSection({ src, today }: { src: ScheduleSource; today: string }) {
  const [showUpload, setShowUpload] = useState(false);

  return (
    <>
      {showUpload && <UploadModal name="Monthly Schedule" onClose={()=>setShowUpload(false)} />}
      <div>
  ...
  useEffect(() => {
    fetch("/api/manage")
      ...
      .then(({ readingSources, scheduleSrc, today }) => {
        // Use only the primary reading source (covers both 1 & 2 readings)
        ...
        <main style={{ flex:1, padding:"36px 40px", overflowY:"auto", minHeight:"100vh" }}>
          {loading ? (
            ...
            <>
              ...
              {section==="reading" && readingSrc && <ReadingSection src={readingSrc} />}
              {section==="schedule" && scheduleSrc && <ScheduleSection src={scheduleSrc} today={today} />}
